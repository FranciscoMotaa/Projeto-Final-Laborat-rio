  ...
  const agendarRota = async () => {
    if (origem && destino) {
      ...
      try {
        ...
        alert('Não foi possível agendar esta rota. Verifique se os locais informados são válidos.')
      } finally {
        setCarregando(false)
      }
    } else {
  ...
  const detectarLocalizacaoAtual = async () => {
    ...
    try {
      ...
      if (endereco) {
        ...
        Alert.alert(
          'Erro',
          'Não foi possível determinar seu endereço atual.',
          [{ text: 'OK' }]
        )
      }
    } catch (error) {
      ...
      >
        ...
        >
          ...
          <View style={styles.rotasPlannerContainer}>
            ...
            </View>              {/* Campo de origem com autocompletar */}
              ...
              {sugestoesOrigem.length > 0 && (
                <View style={styles.sugestoesContainer}>
                  <FlatList
                    ...
                    renderItem={({item}) => renderItemSugestao({item, tipo: 'origem'})}
              ...
              {sugestoesDestino.length > 0 && (
                <View style={styles.sugestoesContainer}>
                  <FlatList
                    ...
                    renderItem={({item}) => renderItemSugestao({item, tipo: 'destino'})}
                    keyExtractor={(item) => item.id.toString()}
                    keyboardShouldPersistTaps="handled"
                    style={styles.sugestoesList}
                    contentContainerStyle={styles.sugestoesContent}
              ...
              >
                {carregando ? (
                  ...
                  <>
                    ...
                    <Text style={styles.primaryButtonText}>Procurar Rota</Text>
                  </>
                )}
              </TouchableOpacity>
            </View>
...
const styles = StyleSheet.create({
  ...
  scrollViewContent: {
    flexGrow: 1,
    paddingBottom: 100, // Adicionando espaço extra no final para quando o teclado abrir
  },
  touchableBackground: {
    flex: 1,
  },
  ...
  // Estilos para o planeador de rotas
  ...
  sugestoesContainer: {
    position: 'absolute',
    top: 48, // Altura do input + padding
    left: 0,
    right: 0,
    backgroundColor: '#fff',
    borderRadius: 8,
